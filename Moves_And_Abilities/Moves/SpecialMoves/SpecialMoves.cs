...
{
    public SpecialMove(string name, int power, int accuracy, int range, PokemonType type) : base(name, power, accuracy, range, type)
    {
    }
    ...
    public override void Execute(Pokemon user, Pokemon target)
    {
        ...
        int baseDamage = (user.Stats.TotalSpecialAttack + Power) - (target.Stats.TotalSpecialDefense / 2);
